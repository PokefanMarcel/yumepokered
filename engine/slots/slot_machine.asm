...
.loop
	...
	jr z, .stopWheel ; marcelnote - fix for aforementioned bug (pokered Wiki)
	dec c
	jr nz, .loop
	ret
.stopWheel
...
IF DEF(_FRA)
	IF DEF(_RED)
		INCBIN "translation/fra/gfx/slots/red_slots_1.2bpp"
	ELIF DEF(_BLUE)
		INCBIN "translation/fra/gfx/slots/blue_slots_1.2bpp"
	ELIF DEF(_GREEN) ; PureRGBnote: GREENBUILD: use the green slots visuals on green version
		INCBIN "translation/fra/gfx/slots/green_slots_1.2bpp"
	ENDC
ELSE
	IF DEF(_RED)
